...
![Dual Axis Line Chart Dataset 3 Question EC](dataset3_ec_excel.png)
...
Two datasets are read from Google Drive, namely, 'life_expectancy_data' and 'mortality_data'. The datasets are merged into a single DataFrame, 'merged_data', using the 'Year' column as the common key. The 'Year' column in the 'merged_data' DataFrame is converted into string data type. This conversion ensures that 'Year' is treated as a string for consistent data type usage and appropriate labeling in plots.
